%% Analysis Script
% Using .csv scripts from evaluation_cube.m computes average performance across
% original and modified algorithms. Takes two input CSV files, so that 
% one original (baseline) file can be used, allowing most evaluation_cube.m
% use cases to skip re-running the original algorithm. 
...
% Ok to have modified algorithm performance in this CSV, it will only use
% the original algorithm's data
...
% Ok to have original algorithm performance in this CSV, it will only use
% the modified algorithm's data
...
%% Sanity check for # of images in CSV 
...
% backwards compatible with older CSVs that don't have fallback flag/label
...
%% Tally # images that are better/worse than original method
tol = 1e-2;
...
fprintf('\nDone.\n');
 ...
 %% Plot sorted ΔE2000 distributions and overlay fallback cases
if HIST && any(strcmp(performance_data.Properties.VariableNames,'FallbackUsed'))
    topN = 120;

    % Get indices of topN worst original DeltaE2000
...
%% Fallback ONLY comparison: original vs modified ΔE2000
if HIST && any(strcmp(performance_data.Properties.VariableNames,'FallbackUsed'))
